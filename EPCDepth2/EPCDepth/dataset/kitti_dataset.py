...
class KittiDataset(Dataset):
    def __init__(self, data_path, img_height, img_width, train, val, split="eigen_full", depth_hint_path=None, use_depth_hint=False, test=False):
        ...
        if self.train:
            filenames_file = os.path.join(filenames_file, "temp_files.txt")
        elif not test:
            filenames_file = os.path.join(filenames_file, "temp_files.txt")
        else:
            filenames_file = os.path.join(filenames_file, "temp_files.txt")
    ...
    def get_img(self, folder, frame_idx, side, loader=pil_loader):
        img_path = os.path.join(folder,"{:010d}{}".format(frame_idx, ".png"))
        img = loader(img_path)
        return img
    ...
    def get_intrinsic_matrix(self, folder, frame_idx):
        folder = ('/').join(folder.split('/')[:-1])
        intrinsic_matrix = os.path.join(folder,'intrinsics',"{:010d}{}".format(frame_idx, ".txt"))
        with open(intrinsic_matrix) as f:
          lines = f.readlines()
        intrinsic_matrix = np.array((lines[0]).split(' ')[:-1]).astype('float64')
    ...
    def transform(self, image, x, is_flip, is_color_aug, color_aug):
        # will be [0,1]
    ...
    def __getitem__(self, index):
        ...
        data["curr"] = self.transform(frame_idx, self.get_img(folder, frame_idx, side), is_flip, False, color_aug)

        data["id"] = index
        data["file_name"] = frame_idx
        if self.train:
            data["side"] = self.side_map[side]
            data["curr_color_aug"] = self.transform(frame_idx, self.get_img(folder, frame_idx, side), is_flip, is_color_aug, color_aug)

            #other_side = {"r": "l", "l": "r"}[side]
            other_side = {"l": "r", "r": "l"}[side]
            data["other_side"] = self.transform(frame_idx, self.get_img(folder, frame_idx, other_side), is_flip, False, color_aug)
            data["other_side_color_aug"] = self.transform(frame_idx, self.get_img(folder, frame_idx, other_side), is_flip, is_color_aug, color_aug)
            ...
            data["intrinsic"] = self.get_intrinsic_matrix(folder, frame_idx)
            # if self.use_depth_hint:
            ...
            if(not(self.val)):
                ...
                data["intrinsic"] = self.get_intrinsic_matrix(folder, frame_idx)
             #data["depth_gt"] = self.get_depth_gt(folder, frame_idx)
