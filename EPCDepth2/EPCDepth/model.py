...
from network.intrinsic_decoder import InstrinsicDecoder
...
class Model:
    def __init__(self, args):
        ...
        if self.args.use_full_scale:
            ...
            self.model["depth_decoder"] = DepthDecoder(num_output_channels=1,
                                                       encoder_layer_channels=self.model["encoder"].layer_channels).to(self.device)
        self.model["intrinsic_decoder"] = InstrinsicDecoder().to(self.device)
        val_dataset = KittiDataset(data_path=self.args.data_path, img_height=self.args.img_height, img_width=self.args.img_width,
                                   train=False, val=self.args.val_depth,split=self.args.split, test=self.args.val)
        ...
        #parameters_to_train = list(self.model["encoder"].parameters()) + list(self.model["intrinsic_decoder"].parameters())
        parameters_to_train = list(self.model["intrinsic_decoder"].parameters())
        ...
        self.save_dir = os.path.join(self.args.models_dir)
    ...
    def main(self):
        ...
        if self.args.resume:
            ...
            for m_name, _ in self.model.items():
                if m_name in checkpoint:
                    print('--------HERE-------------')
                    if(self.args.val_depth):
                        if(m_name != 'intrinsic_decoder'):
                            self.model[m_name].load_state_dict(checkpoint[m_name])
                    else:
                        if(m_name != 'depth_decoder'):
                            self.model[m_name].load_state_dict(checkpoint[m_name])
                else:
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            ...
            save_filename = os.path.join('saved_models', "checkpoint_epoch{}.pth.tar".format(epoch))
    ...
    def validate(self):
        if(self.args.val_depth):
            ...
            for batch, data in enumerate(self.val_loader):
                ...
                features, intrinsic_features = self.model["encoder"](ipt)
                pred_disps = self.model["depth_decoder"](features)
                ...
                all_disps.append(pred_disps.cpu())
                file_name = data["file_name"]
                file_names.append(file_name.cpu())
            ...
            if self.args.val:
                ...
                np.save(os.path.join(self.args.disps_path, "file_names"), file_names)
            ...
            for batch, data in enumerate(self.val_loader):
                ...
                features, intrinsic_features = self.model["encoder"](ipt)
                pred_matrix = self.model["intrinsic_decoder"](intrinsic_features)

                matrix_loss = nn.MSELoss()
                matrix_loss_list.append(matrix_loss(data["intrinsic"].float(), pred_matrix).float())
                
                
                all_matrices.append(pred_matrix.cpu())
                file_name = data["file_name"]
                file_names.append(file_name.cpu())
            ...
            file_names = np.concatenate(file_names)
            matrix_loss_avg = sum(matrix_loss_list)/len(matrix_loss_list)


            if self.args.val:
                for file,matrix in zip(file_names, all_matrices):
                    matrix = str(matrix.tolist())
                    fp= open(r''+self.args.disps_path +'/'+ 'intrinsic/'+ str(file)+'.txt', 'w')
                    fp.write(matrix)

            return matrix_loss_avg
    ...
    def train_epoch(self, epoch):
        ...
        for batch, data in enumerate(self.train_loader):
            ...
            losses_graph.append(loss.detach().cpu())
    ...
    def train_step(self, data):
        predicts = {}
        features, intrinsic_features = self.model["encoder"](data["curr_color_aug"])
        predicts["disparity"] = self.model["depth_decoder"](features)
        predicts["intrinsic"] = self.model["intrinsic_decoder"](intrinsic_features)
        predicts["depth"] = []
        ...
        loss = nn.MSELoss()
        output = loss(data["intrinsic"].float(), predicts["intrinsic"]).float()
        print('File name:',data["file_name"].cpu())
        print('Prediction:',predicts["intrinsic"].cpu())
    ...
    def visualization(self):
        ...
        save_dir = os.path.join(os.path.dirname(self.args.disps_path), "disps_vis2")
        os.makedirs(save_dir, exist_ok=True)
        disps = np.load(self.args.disps_path + "/disparities.npy")
        file_names = np.load(self.args.disps_path + "/file_names.npy", allow_pickle=True)
        for idx, pred_disp in zip(file_names, disps):
